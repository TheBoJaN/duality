	...
	public class CanvasTest
	{
		...
		[Test] public void RenderAllShapesTransformed()
		{
			this.TestImagesEqual(TestRes.CanvasTestAllShapesTransformed, c => 
			{
				...
				c.State.TransformHandle = new Vector2D(5, 5);
				c.State.TransformScale = new Vector2D(0.75f, 0.75f);
		...
		{
			Vector2D[] polygon = new Vector2D[]
			{ 
				new Vector2D(0.0f, 0.0f), 
				new Vector2D(50.0f, 0.0f), 
				new Vector2D(50.0f, 45.0f), 
				new Vector2D(5.0f, 50.0f), 
			};
			...
			c.PushState();
			{
				...
				c.DrawCircleSegment(x, y, 25, 0.0f, MathD.RadAngle30 * 4, true);
				x += 100;
				...
				c.DrawOvalSegment(x, y, 50, 50, 0.0f, MathD.RadAngle30 * 4, true);
				x += 100;
			...
			c.PushState();
			{
				...
				c.FillCircleSegment(x, y, 0, 25, MathD.RadAngle30 * 0, MathD.RadAngle30 * 4);
				c.FillCircleSegment(x, y, 0, 25, MathD.RadAngle30 * 5, MathD.RadAngle30 * 9, 10);
				x += 100;
				...
				c.FillOvalSegment(x, y, 0, 50, 50, MathD.RadAngle30 * 0, MathD.RadAngle30 * 4);
				c.FillOvalSegment(x, y, 0, 50, 50, MathD.RadAngle30 * 5, MathD.RadAngle30 * 9, 10);
				x += 100;
		...
		private bool AreImagesEqual(PixelData first, PixelData second)
		{
			...
			double error = 0;
			double maxError = firstData.Length / 10.0f; // (1/255) off per ten pixels is probably okay.
			for (int i = 0; i < firstData.Length; i++)
			{
				error += MathD.Abs(firstData[i].R * (firstData[i].A / 255.0f) - secondData[i].R * (secondData[i].A / 255.0f));
				error += MathD.Abs(firstData[i].G * (firstData[i].A / 255.0f) - secondData[i].G * (secondData[i].A / 255.0f));
				error += MathD.Abs(firstData[i].B * (firstData[i].A / 255.0f) - secondData[i].B * (secondData[i].A / 255.0f));
				error += MathD.Abs(firstData[i].A - secondData[i].A);
		...
		private PixelData CreateDiffImage(PixelData first, PixelData second)
		{
			...
			for (int i = 0; i < firstData.Length; i++)
			{
				diffData[i].R = (byte)MathD.Abs(firstData[i].R - secondData[i].R);
				diffData[i].G = (byte)MathD.Abs(firstData[i].G - secondData[i].G);
				diffData[i].B = (byte)MathD.Abs(firstData[i].B - secondData[i].B);
				diffData[i].A = (byte)MathD.Abs(firstData[i].A - secondData[i].A);
			}
